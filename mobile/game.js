...
const CANVAS_W    = 760;
const CANVAS_H    = 480;
// Динамические размеры (обновляются при старте игры)
let CANVAS_W_DYN = 760;
let CANVAS_H_DYN = 480;
const CW = () => CANVAS_W_DYN;
const CH = () => CANVAS_H_DYN;
...
let particles = [];
let orbs = [];
let CANVAS_W_CUR = 760;
let CANVAS_H_CUR = 480;
...
function startGame() {
  ...
  // Адаптивный размер — на мобильных под экран
  const isMob = window.innerWidth <= 900;
  const cw = isMob ? Math.min(window.innerWidth, 760) : 760;
  const ch = isMob ? Math.min(window.innerHeight - 260, 420) : 480;
  canvas.width  = cw;
  canvas.height = ch;
  // Обновляем константы под реальный размер
  CANVAS_W_CUR = cw;
  CANVAS_H_CUR = ch;
  CANVAS_W_DYN = cw;
  CANVAS_H_DYN = ch;
...
function update(dt) {
  // Player blocking
  player.blocking = !!keys["KeyF"] || (typeof isTbDown === "function" && isTbDown("block"));
  if (player.blocking) {
  ...
  if (player.dashTimer > 0) {
    player.dashTimer -= dt;
    player.x = clamp(player.x + player.dashVx, RADIUS, CANVAS_W_CUR - RADIUS);
    player.y = clamp(player.y + player.dashVy, RADIUS, CANVAS_H_CUR - RADIUS);
    ...
    if (dd < RADIUS * 2.2 && bot.iframeTimer <= 0) {
      ...
      bot.x = clamp(bot.x + nx * 20, RADIUS, CANVAS_W_CUR - RADIUS);
      bot.y = clamp(bot.y + ny * 20, RADIUS, CANVAS_H_CUR - RADIUS);
    ...
    player.x = clamp(player.x + player.vx, RADIUS, CANVAS_W_CUR - RADIUS);
    player.y = clamp(player.y + player.vy, RADIUS, CANVAS_H_CUR - RADIUS);
  }
  // Bot dash
  if (bot.dashTimer > 0) {
    bot.dashTimer -= dt;
    bot.x = clamp(bot.x + bot.dashVx, RADIUS, CANVAS_W_CUR - RADIUS);
    bot.y = clamp(bot.y + bot.dashVy, RADIUS, CANVAS_H_CUR - RADIUS);
    ...
    if (dd < RADIUS * 2.2 && player.iframeTimer <= 0) {
      if (!player.blocking) {
        ...
        player.x = clamp(player.x + nx * 20, RADIUS, CANVAS_W_CUR - RADIUS);
        player.y = clamp(player.y + ny * 20, RADIUS, CANVAS_H_CUR - RADIUS);
...
function updateBot(dt) {
  ...
  if (lowHp && dist < bs.aggroDist) {
    ...
    if (dist < safeDist) {
      ...
      bot.x = clamp(bot.x + (-dy / dist) * spd * side * bs.strafeSpeed, RADIUS, CANVAS_W_CUR - RADIUS);
      bot.y = clamp(bot.y + ( dx / dist) * spd * side * bs.strafeSpeed, RADIUS, CANVAS_H_CUR - RADIUS);
    }
    ...
    bot.x = clamp(bot.x + mx, RADIUS, CANVAS_W_CUR - RADIUS);
    bot.y = clamp(bot.y + my, RADIUS, CANVAS_H_CUR - RADIUS);
  }
}
...
function _botMove(e, dx, dy, dist, spd) {
  ...
  e.x = clamp(e.x + mx, RADIUS, CANVAS_W_CUR - RADIUS);
  e.y = clamp(e.y + my, RADIUS, CANVAS_H_CUR - RADIUS);
}
...
function botAttack() {
  ...
  player.x = clamp(player.x + (dx / dist) * KNOCKBACK, RADIUS, CANVAS_W_CUR - RADIUS);
  player.y = clamp(player.y + (dy / dist) * KNOCKBACK, RADIUS, CANVAS_H_CUR - RADIUS);
  updateHUD();
...
function playerAttack() {
  ...
  bot.x = clamp(bot.x + (dx / dist) * KNOCKBACK, RADIUS, CANVAS_W_CUR - RADIUS);
  bot.y = clamp(bot.y + (dy / dist) * KNOCKBACK, RADIUS, CANVAS_H_CUR - RADIUS);
  updateHUD();
